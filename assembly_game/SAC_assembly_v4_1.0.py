...
from tianshou.policy import  MultiAgentPolicyManager, ICMPolicy, PPOPolicy, DQNPolicy,DiscreteSACPolicy
...
if __name__ == "__main__":
    ...
    train_env = DummyVectorEnv([lambda: make_env() for _ in range(1)])
    ...
    actor_optim1 = torch.optim.Adam(actor1.parameters(), lr=args.lr)
    critic1 = Critic(net1, last_size=action_shape, device=args.device)
    critic1_optim = torch.optim.Adam(critic1.parameters(), lr=args.lr)
    critic2 = Critic(net1, last_size=action_shape, device=args.device)
    critic2_optim = torch.optim.Adam(critic2.parameters(), lr=args.lr)
    policy1: DiscreteSACPolicy = DiscreteSACPolicy(
        actor=actor1,
        actor_optim=actor_optim1,
        critic=critic1,
