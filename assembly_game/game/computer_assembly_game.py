...
class ComputerAssemblyGame:
    def __init__(self):
        ...
        self.fail_action_reward = -1
        self.assemble_success_reward = 10
        self.total_step = 0
    ...
    def reset_game(self, seed):
        ...
        for component in ['motherboard', 'CPU', 'cooler', 'GPU', 'memory_card', 'power_supply', 'hard_disk', 'cover']:
    ...
    def move_hands_simultaneously(self, action_hand1, action_hand2):
        ...
        component_id1 = component_id2 = None
        last_direction1 = last_direction2 = ''
        steps_hand1 = steps_hand2 = 0

        # Check for handover action for hand1
        if action_hand1 == 10:
            if self.hand2_waiting_for_handover:
                # Implement handover logic here
                target_x1, target_y1 = self.case_x, self.case_y
            else:
                # print("Handover from Hand1 to Hand2 failed. Hand2 was not waiting for handover.")
                self.reward += self.fail_action_reward

        # Handling for moving to components (actions 1-8) remains unchanged
        if 1 <= action_hand1 <= 8:
            component_id1 = action_hand1 - 1
            target_x1, target_y1 = self.components[component_id1].x, self.components[component_id1].y
            # Check if action_hand2 is to do nothing
        # if action_hand2 == 10:
            # print("Hand2 does nothing.")
        elif 1 <= action_hand2 <= 8:
            component_id2 = action_hand2 - 1
            target_x2, target_y2 = self.components[component_id2].x, self.components[component_id2].y

        # New handling for action 9 - Move to the case
        if action_hand1 == 9:
            target_x1, target_y1 = self.case_x, self.case_y  # Set target to case position for Hand1
        if action_hand2 == 9:
            target_x2, target_y2 = self.case_x, self.case_y  # Set target to case position for Hand2

        while target_x1 is not None or target_x2 is not None:
            if target_x1 is not None:
                self.hand_x, self.hand_y, step_made1, last_direction1 = self.move_towards(
                    self.hand_x, self.hand_y, target_x1, target_y1, last_direction1)
                steps_hand1 += step_made1
                if self.hand_x == target_x1 and self.hand_y == target_y1:
                    if action_hand1 == 9:
                        self.handle_case_interaction(1)  # Special handling for the case
                    elif action_hand1 ==10:
                        self.handover()
                    else:
                        self.pick_up_component(component_id1, 1)
                    target_x1 = None  # Target reached

            if target_x2 is not None:
                self.second_hand_x, self.second_hand_y, step_made2, last_direction2 = self.move_towards(
                    self.second_hand_x, self.second_hand_y, target_x2, target_y2, last_direction2)
                steps_hand2 += step_made2
                if self.second_hand_x == target_x2 and self.second_hand_y == target_y2:
                    if action_hand2 == 9:
                        self.handle_case_interaction(2)  # Special handling for the case
                    else:
                        self.pick_up_component(component_id2, 2)
                    target_x2 = None  # Target reached


        #  print(f"Hand1 moved {steps_hand1} steps" + (
        #     " and interacted with the case." if action_hand1 == 9 else f" and picked up component {action_hand1}.") if component_id1 is not None else "Hand1 did not pick up a component or interact with the case.")
        # print(f"Hand2 moved {steps_hand2} steps" + (
        #     " and interacted with the case." if action_hand2 == 9 else f" and picked up component {action_hand2}.")
        #       if component_id2 is not None else "Hand2 did not pick up a component or interact with the case.")
        self.update()
        # self.render()
        self.total_step += steps_hand1
    ...
    def handle_case_interaction(self, hand_id):
        """
        Handles interaction with the case for a specific hand.
        """
        # This could involve setting the hand to wait for a handover, assembling components, etc.
        # For demonstration, just print the interaction:
        if hand_id == 1:
            for comp in self.components:
    ...
    def move_towards(self, start_x, start_y, target_x, target_y, last_direction):
        ...
        if last_direction == 'y' or last_direction == '':  # Prioritize x-direction first if last move was y or if starting
            if start_x < target_x:
                ...
                new_direction = 'x'
        if (
                start_y < target_y or start_y > target_y) and not step_made:  # Then move in y if x didn't change or was not prioritized
            if start_y < target_y:
        ...
        return start_x, start_y, 1 if step_made else 0, new_direction
